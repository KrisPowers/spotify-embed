// worker.ts — deploy this as a standalone Cloudflare Worker
// It handles token refresh + SVG generation entirely independently of Pages.
//
// Setup:
//   1. npx wrangler deploy worker.ts --name spotify-now-playing --compatibility-date 2024-01-01
//   2. Set secrets:
//        npx wrangler secret put SPOTIFY_CLIENT_ID
//        npx wrangler secret put SPOTIFY_CLIENT_SECRET
//        npx wrangler secret put SPOTIFY_REFRESH_TOKEN
//   3. Embed in any README:
...
function svgPlaying(track: string, artists: string, art: string, progressMs: number, durationMs: number): string {
  const W = 480, H = 130;
...
// Cache-bust Cloudflare's own edge cache by making the URL unique per-request.
// We do this by redirecting from the root URL to a timestamped URL,
// but serving the SVG directly on timestamped requests.
export default {
  async fetch(request: Request, env: Env): Promise<Response> {
    const url = new URL(request.url);

    // Always respond with fresh SVG — Workers bypass CF edge cache by default
    const headers = {
