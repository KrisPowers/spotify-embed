...
export function pageTopArtists(origin: string): string {
  const body = `
    <div class="page-header">
      <h1 class="page-title">Top Artists</h1>
      <p class="page-subtitle">A grid of your most-listened artists with artwork, rank, and genre. Configure the time range and how many to show.</p>
    </div>
    ...
    <div class="section">
      ...
      <div class="preview-wrap" id="preview-wrap">
        <img id="preview-img" src="${origin}/top-artists.svg?range=short_term&count=8&t=${Date.now()}" alt="Top Artists preview"/>
      </div>
    </div>
    ...
    <script>
      ...
      function updatePreview() {
        const url = '${origin}/top-artists.svg?range=' + currentRange + '&count=' + currentCount + '&t=' + Date.now();
        document.getElementById('preview-img').src = url;
        document.getElementById('embed-code').textContent =
          '![Top Artists](${origin}/top-artists.svg?range=' + currentRange + '&count=' + currentCount + ')';
      }
  ...
  return htmlShell("Top Artists", body, "top-artists");
}
