...
export function svgNowPlaying(
  ...
  const displayTrack = esc(trunc(track, 20));
  const displayArtists = esc(trunc(artists, 16));
  const displayAlbum = esc(trunc(album, 13));
  ...
  const vinylRings = [16, 26, 36, 46, 56, 66, 74].map((r, i) => {
  ...
  <g clip-path="url(#card-clip)">
    ...
    <text x="56" y="55" font-size="18" fill="#f5f5f5" font-weight="700">Now Playing</text>
    <text x="28" y="89" font-size="12.8" fill="#ffffffde">
    ...
    <g filter="url(#disk-shadow)">
      <g>
        ...
        ${vinylRings}
        <circle cx="${diskCx}" cy="${diskCy}" r="23" fill="#333841"/>
        <circle cx="${diskCx}" cy="${diskCy}" r="8.5" fill="#0f1013"/>
