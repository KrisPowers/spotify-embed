...
function computeListLayout(format: SocialFormat): { topPad: number; sidePad: number; rowHeight: number; rowGap: number; maxItems: number; titleSize: number; subtitleSize: number } {
  ...
  if (format === "portrait") {
    return { topPad: 300, sidePad: 82, rowHeight: 150, rowGap: 18, maxItems: 6, titleSize: 34, subtitleSize: 24 };
  }
  return { topPad: 236, sidePad: 76, rowHeight: 132, rowGap: 16, maxItems: 6, titleSize: 30, subtitleSize: 22 };
}
...
export function svgSocialCard(input: SocialCardRenderInput): string {
  ...
  <text x="${size.width - 80}" y="${size.height - 78}" font-size="24" fill="#696969" text-anchor="end">${esc(getCanvasSize(format).label)}${dateLabel ? ` - ${dateLabel}` : ""}</text>
</svg>`;
}
